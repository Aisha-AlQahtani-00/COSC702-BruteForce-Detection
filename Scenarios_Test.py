...
def run_scenario(scenario_name, file_path, loaded_models, vectorizer):
    ...
    #Load labeled data if available (AFTER status checks, df is guaranteed to exist here so len(df) is safe.
    labeled_df   = None
    ...
    for model_name, model in loaded_models.items():
        ...
        #skip F1 for Healthy Logs (All records are Normal so F1 is meaningless here).
        metrics = None
...
def plot_heatmap(results_df):
    ...
    #Enforce consistent row and column ordering.
...
def plot_detection_consistency(results_df):
    ...
    Shows whether models stay consistent as data volume grows flat lines = reliable model, erratic lines = unstable model.
...
#Updated plot after the Apr 28 discussion and feedback.
...
def export_dataset_table():
    ...
    #Testing scenarios.
    ('Dataset1k', 'Testing', '1,000', 'Labeled', 'Small scale scenario test'),
    ('Dataset15k', 'Testing', '15,000', 'Labeled', 'Medium scale scenario test'),
    ('Dataset20k', 'Testing', '20,000', 'Labeled', 'Large scale scenario test'),
    ('Dataset100k', 'Testing', '100,000', 'Labeled', 'Very large scale scenario test'),
    ('Healthy Logs Sample', 'Testing', '~50', 'Labeled', 'False positive evaluation'),
